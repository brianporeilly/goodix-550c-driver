...
/**
 * Send raw TLS handshake/record bytes to the 550c inside a 0xB0 pack and
 * advance @ssm on completion (GOODIX_VARIANT_TLS_PSK only).
 */
void goodix_send_tls_pack (FpiSsm       *ssm,
                           FpDevice     *dev,
                           const guint8 *raw,
                           gsize         raw_len);
