...
/**
 * goodix_proto_wrap_pack:
 *
 * Wrap @payload in a 550c pack: [flag][len:2 LE][cksum][payload], where
 * cksum = (flag + len_lo + len_hi) & 0xFF. Mirrors goodix.py
 * encode_message_pack().
 */
guint8 *
goodix_proto_wrap_pack (guint8        flag,
                        const guint8 *payload,
                        gsize         payload_len,
                        gsize        *out_len)
{
  gsize total = 4 + payload_len;
  guint8 *pack = g_malloc (total);
  guint16 len_field = (guint16) payload_len;

  pack[0] = flag;
  pack[1] = len_field & 0xFF;
  pack[2] = (len_field >> 8) & 0xFF;
  pack[3] = (guint8) ((pack[0] + pack[1] + pack[2]) & 0xFF);

  if (payload_len > 0)
    memcpy (pack + 4, payload, payload_len);
                            ...
                            gsize             chunk_len)
{
  ...
  /*
   * 550c pack framing: chunks are concatenated raw (no continuation markers);
   * the pack header gives the total length.
   *   [flag][payload_len:2 LE][checksum][payload]
   */
  if (rx->pack_layer)
    {
      ...
      if (rx->expected == 0 && rx->len >= 4)
        {
          rx->pack_flag = rx->buf[0];
          guint16 pack_len = rx->buf[1] | ((guint16) rx->buf[2] << 8);
          rx->expected = (gsize) pack_len + 4;
        }
      return TRUE;
    }
...
/**
 * goodix_proto_rx_pack:
 *
 * Return the flag and raw payload of a completed pack (pack_layer only).
 * Validates the pack checksum.
 */
gboolean
goodix_proto_rx_pack (GoodixReassembly *rx,
                      guint8           *out_flag,
                      ...
                      gsize            *out_payload_len)
{
  ...
  if (!rx->pack_layer || !goodix_proto_rx_complete (rx) || rx->expected < 4)
    return FALSE;

  computed = (guint8) ((rx->buf[0] + rx->buf[1] + rx->buf[2]) & 0xFF);
  if (computed != rx->buf[3])
    {
      fp_warn ("Pack checksum mismatch: computed 0x%02x, got 0x%02x",
               computed, rx->buf[3]);
      return FALSE;
    }

  *out_flag = rx->buf[0];
  *out_payload = rx->buf + 4;
  *out_payload_len = rx->expected - 4;
  return TRUE;
}
                       ...
                       gsize            *out_payload_len)
{
  ...
  if (rx->pack_layer)
    {
      /* Unwrap the 550c pack; the inner payload must be a message frame. */
      guint8 flag;
      const guint8 *pack_payload;
      gsize pack_payload_len;

      if (!goodix_proto_rx_pack (rx, &flag, &pack_payload, &pack_payload_len))
        return FALSE;

      if (flag != GOODIX_PACK_FLAG_MESSAGE)
        {
          fp_warn ("Expected a message pack (0xA0), got flag 0x%02x", flag);
          return FALSE;
        }

      msg = pack_payload;
      msg_len = pack_payload_len;
    }
  else
    {
      msg = rx->buf;
